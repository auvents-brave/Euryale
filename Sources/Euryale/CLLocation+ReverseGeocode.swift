...
extension CLLocation {
    ...
    public func Name() async -> String {
        if #available(macOS 26, iOS 26, tvOS 26, watchOS 26, visionOS 26, *) {
            guard let descriptor = await ReverseGeocode26(), let address = descriptor.address, let first = address.components(separatedBy: .newlines).first else { return "" }
            return first
        } else {
        ...
        return await withCheckedContinuation { continuation in
            CLGeocoder().reverseGeocodeLocation(self) { placemarks, error in
                guard error == nil else {
                    Logger(label: "").error("Reverse geocoding failed", metadata: ["error": "\(error!.localizedDescription)"])
    ...
    fileprivate func ReverseGeocode26() async -> PlaceDescriptor? {
        let request = MKReverseGeocodingRequest(location: self)
        do {
            guard let mapItems = try await request?.mapItems, let firstItem = mapItems.first else { return nil }
    ...
    for (index, data) in locations.enumerated() {
        ...
        if let region = placemark.region {
            ...
            if let circularRegion = region as? CLCircularRegion {
                print("  - Region Center: \(circularRegion.center.latitude), \(circularRegion.center.longitude)")
        ...
        print("  - Supporting Representations: \(descriptor.supportingRepresentations.count) available")
        ...
        for rep in descriptor.representations {
            ...
            case let .address(addr):
                print("    • Address: \(addr)")
            case let .deviceLocation(device):
                print("    • Device: \(device)")
            @unknown default:
