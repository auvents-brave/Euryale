...
internal struct DisplayName {
  static func getDisplayName(_ bundle: any BundleInfoProviding) -> String {
    guard
      let localizedDisplayName = bundle.localizedInfoDictionary?["CFBundleDisplayName"] as? String
    else {
      guard let displayName = bundle.infoDictionary?["CFBundleDisplayName"] as? String else {
        guard let localizedBundleName = bundle.localizedInfoDictionary?["CFBundleName"] as? String
        else {
