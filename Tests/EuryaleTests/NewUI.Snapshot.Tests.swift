	...
	struct NewUISnapshotTests {
		...
		@Test func `icon label`() async throws {
			let view = VStack(alignment: .leading, spacing: 12) {
				...
				IconLabel(
					"Routes" as String, systemImage: "point.topleft.down.to.point.bottomright.curvepath")
			}
		...
		@Test func `scattered photos view`() async throws {
			let view = ScatteredPhotosView(images: [
				solidImage(.red), solidImage(.green), solidImage(.blue),
			])
