...
public struct AdaptiveToolbar: View {
	...
	private let collapsedAsMenu: Bool
	...
	///   - collapsedAsMenu: When `true`, the whole toolbar renders as a single
	///     glass "…" button that presents all actions in a menu (macOS), popover
	///     (iOS/iPadOS/visionOS) or confirmation dialog (tvOS/watchOS). Use it for
	///     a compact, space-stable control that does not grow as actions are added.
	public init(
		_ actions: [ToolbarAction],
		showsBackground: Bool = true,
		allowsOverflow: Bool = true,
		collapsesToIcons: Bool = true,
		collapsedAsMenu: Bool = false
	) {
		self.groups = [actions]
		self.showsBackground = showsBackground
		self.allowsOverflow = allowsOverflow
		self.collapsesToIcons = collapsesToIcons
		self.collapsedAsMenu = collapsedAsMenu
	}
	...
	///   - collapsedAsMenu: When `true`, the whole toolbar renders as a single
	///     glass "…" button that presents all actions in a menu / popover /
	///     confirmation dialog (see the other initialiser).
	public init(
...
private struct OverflowButton: View {
	...
	var body: some View {
			...
			// No `Menu`/`popover` here; present the actions in a confirmation dialog.
			Button {
				isPresented = true
			} label: {
				ellipsis
			...
			VStack(alignment: .leading, spacing: 2) {
				ForEach(actions) { action in
					Button(role: action.role) {
						...
						Label(action.title, systemImage: action.systemImage)
							.foregroundStyle(
								action.role == .destructive ? AnyShapeStyle(.red) : AnyShapeStyle(.primary)
							)
