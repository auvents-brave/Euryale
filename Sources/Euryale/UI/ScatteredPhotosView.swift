...
public struct ScatteredPhotosView: View {
    ...
    private static func layout(index: Int, seed: UInt64, maxAngle: Double) -> Layout {
        var rng = SeededGenerator(seed &+ UInt64(bitPattern: Int64(index)) &* 0x100000001B3)
...
private struct PhotoCard: View {
    ...
    private var curlShading: some View {
        ZStack {
            ...
            LinearGradient(colors: [.white.opacity(0.22 * curl), .clear], startPoint: .top, endPoint: .center)
            VStack(spacing: 0) {
                Spacer()
                LinearGradient(colors: [.clear, .black.opacity(0.30 * curl)], startPoint: .top, endPoint: .bottom)
                    .frame(height: size.height * 0.20)
                LinearGradient(colors: [.white.opacity(0.65 * curl), .white.opacity(0.05 * curl)], startPoint: .bottom, endPoint: .top)
