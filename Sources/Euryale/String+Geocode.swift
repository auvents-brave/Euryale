...
extension String {
    ...
    fileprivate nonisolated func Geocode00(near: CLLocationCoordinate2D?) async -> CLLocationCoordinate2D? {
        return await withCheckedContinuation { continuation in
            CLGeocoder().geocodeAddressString(self) { placemarks, error in
                guard error == nil else {
                    Logger(label: "").error("Forward geocoding failed", metadata: ["error": "\(error!.localizedDescription)"])
    ...
    /// reference is given.
    fileprivate static func nearest(_ coordinates: [CLLocationCoordinate2D], to reference: CLLocationCoordinate2D?) -> CLLocationCoordinate2D? {
